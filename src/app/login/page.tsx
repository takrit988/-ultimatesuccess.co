...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (errorParam === "CredentialsSignin") {
      setError(t("Invalid email or password", "อีเมลหรือรหัสผ่านไม่ถูกต้อง"));
    } else if (errorParam) {
      setError(t("Authentication failed", "เกิดข้อผิดพลาดในการตรวจสอบสิทธิ์"));
    }
  }, [searchParams]);
      ...
      <div className="max-w-md w-full bg-slate-950/80 border border-gold-500/10 rounded-2xl p-8 backdrop-blur-md shadow-2xl space-y-6 relative z-10">
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
            {t("Partner Portal Login", "เข้าสู่ระบบพอร์ทเนอร์")}
          </h1>
          <p className="text-xs text-slate-400 font-light">
            {t("Access executive learning tools and insights.", "เข้าเรียนหลักสูตรและตรวจสอบรายงานของคุณ")}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="relative flex items-center justify-center my-2">
            ...
            <span className="relative px-3 bg-slate-950 text-[10px] text-slate-500 uppercase tracking-widest">
              {t("Or continue with", "หรือเข้าใช้งานด้วย")}
            </span>
          </div>
...
export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-[85vh] bg-slate-900 text-slate-400 dark-premium-gradient">
